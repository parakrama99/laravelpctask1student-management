...
<body>
    {{-- tailwindd css inside container --}}
    <div class="container p-6 mx-auto">
        ...
        {{-- store data after clicking submit or save button --}}
        <form action="{{ route('students.store') }}" method="POST" class="p-6 bg-white rounded-lg shadow-md">
            @csrf {{-- CSRF token for security --}}

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" required class="block w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="mb-4">
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" required class="block w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">

                {{-- Display email validation error given by laravel default  The email has already been taken. --}}
                @error('email')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>

            <button type="submit" class="px-4 py-2 font-semibold text-white transition duration-200 bg-blue-500 rounded-md hover:bg-blue-600">Save</button>
        </form>
    </div>
